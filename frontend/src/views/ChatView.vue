      ...
      <div class="w-full">
        ...
        <div class="flex items-center justify-between px-4 pb-2">
          <p class="text-xs text-[#606060]">
            连接到 <span class="text-[#7c6af7]">{{ chatStore.wsUrl }}</span>
          </p>
          <p class="text-xs" :class="apiStatus === 'ok' ? 'text-green-500' : 'text-red-500'">
            API: {{ apiStatus }}
          </p>
        </div>
      </div>
...
// ===== WebSocket =====
const { status: wsStatus, connect, send, onMessage, disconnect } = useWebSocket();
...
async function sendMessage(text) {
  ...
  if (!sent) {
    chatStore.finishLastAI('⚠️ 发送失败：WebSocket 未连接，请检查 Agent 服务是否启动');
    chatStore.isLoading = false;
  }
}
