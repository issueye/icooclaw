<template>
    <div class="flex w-full h-screen bg-bg-primary overflow-hidden">
        ...
        <ChatSidebar :sessions="chatStore.sessions" :current-session-id="chatStore.currentSessionId"
            ...
            @delete="handleDeleteSession" @toggle="sidebarCollapsed = !sidebarCollapsed" />
        ...
        <div class="flex flex-col flex-1 min-w-0 h-full">
            ...
            <!-- 消息列表 -->
            <div ref="messagesContainer" class="flex-1 overflow-y-auto py-2" :class="chatStore.currentMessages.length === 0
                ? 'flex flex-col items-center justify-center'
                : ''
                ">
                <!-- 欢迎空状态 -->
                <div v-if="chatStore.currentMessages.length === 0" class="text-center px-4 max-w-2xl">
                    <div
                    ...
                    <div class="grid grid-cols-1 gap-2 text-left">
                        <button v-for="hint in hints" :key="hint" @click="sendMessage(hint)"
                ...
                <div v-else class="w-full">
                    <ChatMessage v-for="msg in chatStore.currentMessages" :key="msg.id" :message="msg" />
                </div>
            </div>

            <!-- 输入区 -->
            <div class="w-full">
                <ChatInput ref="chatInputRef" :disabled="chatStore.isLoading" @send="sendMessage" />
                <div class="flex items-center justify-between px-4 pb-2">
                    <p class="text-xs text-text-muted">
                        连接到
                        <span class="text-accent">{{ chatStore.wsUrl }}</span>
                    </p>
                    <p class="text-xs" :class="apiStatus === 'ok'
                        ? 'text-green-500'
                        : 'text-red-500'
                        ">
                        API: {{ apiStatus }}
